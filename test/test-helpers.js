...
function cleanTables(db) {
  return db.transaction((trx) =>
    trx
      ...
      .then(() =>
        Promise.all([
          trx.raw(`ALTER SEQUENCE aie_articles_id_seq minvalue 0 START WITH 1`),
          trx.raw(`ALTER SEQUENCE aie_users_id_seq minvalue 0 START WITH 1`),
          trx.raw(`ALTER SEQUENCE aie_comments_id_seq minvalue 0 START WITH 1`),
          trx.raw(`SELECT setval('aie_articles_id_seq', 0)`),
          trx.raw(`SELECT setval('aie_users_id_seq', 0)`),
          trx.raw(`SELECT setval('aie_comments_id_seq', 0)`),
        ]),
      ),
  )
}
...
function seedUsers(db, users) {
  ...
  return db
    ...
    .then(() =>
      ...
      db.raw(`SELECT setval('aie_users_id_seq', ?)`, [
...
function seedArticlesTables(db, users, articles, comments = []) {
  ...
  return db.transaction(async (trx) => {
    ...
    await trx.into('aie_articles').insert(articles)
    // update the auto sequence to match the forced id values
    await trx.raw(`SELECT setval('aie_articles_id_seq', ?)`, [
      articles[articles.length - 1].id,
    ])
    // only insert comments if there are some, also update the sequence counter
    if (comments.length) {
      await trx.into('aie_comments').insert(comments)
      await trx.raw(`SELECT setval('aie_comments_id_seq', ?)`, [
...
function seedMaliciousArticle(db, user, article) {
  return seedUsers(db, [user]).then(() =>
    db.into('aie_articles').insert([article]),
  )
}
